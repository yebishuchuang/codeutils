            ...
            if (FileUtil.isTemplatFile(t.getAllPath())) {
                ...
                if (t.getFileName().indexOf("[table]") >= 0 || t.getFileName().indexOf("[Table]") >= 0) {
                    for (Table table : tableList) {
                        ...
                        /*
                         * for(String k:publicMap.keySet() ){ String value =
                         * publicMap.get(k).replace("[table]", table.getName());
                         * publicMap.put(k,value ); System.out.println("key:"+k+
                         * "  value:"+value); }
                         */
                        ...
                        // 输出的路径(经过全局替换)
                        String outPath = ClientTempletUtil.createContent(codePath + "/" + t.getPath() + "/" + outFile,
                                publicMap);
